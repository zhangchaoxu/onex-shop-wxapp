Component({
  ...
  methods: {
    ...
    initDraw(init) {
      ...
      this.data.startPercentage = this.data.startPercentage > this.data.percentage ? 0 : this.data.startPercentage;
    ...
    drawProgressCircular() {
      ...
      if (!ctx) {
        ...
        const res = wx.getSystemInfoSync();
				if (!this.data.gradualColor && res.platform.toLocaleLowerCase() == 'android') {
					gradient.addColorStop('1', this.data.progressColor);
				}
        this.setData({
          progressContext: ctx,
      ...
      let time = this.data.percentage == 0 || this.data.duration < 50 ? 0 : this.data.duration / this.data.percentage;
