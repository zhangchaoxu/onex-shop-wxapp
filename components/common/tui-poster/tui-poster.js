const poster = {
	...
	drawPoster(canvasId, winWidth, winHeight, imgs, text, name, callback) {
		...
		// 主图
		let width = poster.rpx2px(648),
			height = poster.rpx2px(774);
		context.drawImage(imgs.mainPic, (winWidth - width) / 2, poster.rpx2px(60), width, height)
		let a = poster.rpx2px(40) //定义行高
		context.setFontSize(poster.rpx2px(30))
		context.setFillStyle("#f00")
		//名称很长调用方法将文字折行，传参 文字内容text，画布context
		let w = poster.getTextWidth(text, context) || poster.rpx2px(550) * 2
		let row = poster.wrapText(text, Math.floor(w / 2), context)
		for (let i = 0; i < row.length; i++) {
			context.fillText(row[i], (winWidth - w / 2) / 2, (winHeight + poster.rpx2px(460)) / 2 + a * i)
		}
		// 识别小程序二维码 
		context.setFontSize(poster.rpx2px(36))
		let w1 = poster.getTextWidth(name, context) || 140
		let x = (winWidth - w1 - 20 - 80) / 2;
		context.drawImage(imgs.qrcode, x, (winHeight + poster.rpx2px(760)) / 2, 80, 80)
		context.setFillStyle("#152338")
		context.fillText(name, x + 80 + 20, (winHeight + poster.rpx2px(890)) / 2)
		context.setFillStyle("#333")
		context.setFontSize(poster.rpx2px(24))
		context.fillText('长按识别·立即体验', x + 80 + 20, (winHeight + poster.rpx2px(990)) / 2)
		context.draw(false, () => {
	...
	drawGoodsPoster(canvasId, winWidth, winHeight, imgs, text, price, originalPrice, name, callback) {
		...
		context.fillRect(0, 0, winWidth, winHeight);
		// 主图
		let width = poster.rpx2px(500 * scaleRatio),
			height = poster.rpx2px(500 * scaleRatio);
		context.drawImage(imgs.mainPic, (winWidth - width) / 2, poster.rpx2px(30 * scaleRatio), width, height)

		let a = poster.rpx2px(40 * scaleRatio) //定义行高
		context.setFontSize(poster.rpx2px(30 * scaleRatio))
		context.setFillStyle("#343434")
		let w = poster.rpx2px(468 * scaleRatio)
		//名称很长调用方法将文字折行，传参 文字内容text，画布context
		let row = poster.wrapText(text, Math.floor(w), context, 2)
		for (let i = 0; i < row.length; i++) {
			context.fillText(row[i], poster.rpx2px(30 * scaleRatio), poster.rpx2px(580 * scaleRatio) + a * i)
		}
		context.setFillStyle("#EB0909")
		context.setFontSize(poster.rpx2px(26 * scaleRatio))
		context.fillText('￥', poster.rpx2px(30 * scaleRatio), poster.rpx2px(680 * scaleRatio))
		let priceArr = Number(price).toFixed(2).toString().split('.')
		context.setFillStyle("#EB0909")
		context.setFontSize(poster.rpx2px(36 * scaleRatio))
		context.fillText(priceArr[0], poster.rpx2px(56 * scaleRatio), poster.rpx2px(680 * scaleRatio))
		let w1 = poster.getTextWidth(priceArr[0], context) || 35
		context.setFontSize(poster.rpx2px(26 * scaleRatio))
		context.setFillStyle("#EB0909")
		context.fillText(`.${priceArr[1]}`, poster.rpx2px(60 * scaleRatio) + w1, poster.rpx2px(680 * scaleRatio))
		context.setFillStyle("#999999")
		context.setFontSize(poster.rpx2px(24 * scaleRatio))
		let w2 = poster.rpx2px(76 * scaleRatio) + w1 + (poster.getTextWidth(`.${priceArr[1]}`, context) || 32)
		context.fillText(`￥${originalPrice}`, w2, poster.rpx2px(680 * scaleRatio))
		context.moveTo(w2, poster.rpx2px(672 * scaleRatio))
		context.lineTo((w2 + 46 * scaleRatio), poster.rpx2px(672 * scaleRatio))
		context.setStrokeStyle('#999999')
		context.stroke(); //对当前路径进行描边
		// 识别小程序二维码 
		let x = winWidth - poster.rpx2px(46 + 130) * scaleRatio;
		context.drawImage(imgs.qrcode, x, poster.rpx2px(735 * scaleRatio), poster.rpx2px(130 * scaleRatio), poster.rpx2px(130 *
			scaleRatio))
		context.setFillStyle("#333")
		context.setFontSize(poster.rpx2px(32 * scaleRatio))
		context.fillText(name, poster.rpx2px(40 * scaleRatio), poster.rpx2px(780 * scaleRatio))
		context.setFontSize(poster.rpx2px(24 * scaleRatio))
		context.fillText('长按识别·立即体验', poster.rpx2px(40 * scaleRatio), poster.rpx2px(835 * scaleRatio))
		context.draw(false, () => {
			poster.createPoster(canvasId, winWidth, winHeight, (res) => {
				callback && callback(res)
			})
